  ...
  return (
    <>
      ...
      <main className="pt-28 pb-20">
        <article>
          <header className="relative bg-surface py-20 md:py-32 border-b border-border/40">
            <div className="container-x max-w-4xl">
              ...
              <p className="mt-6 text-lg text-foreground/80 max-w-3xl leading-relaxed">{article.excerpt}</p>
          ...
          <div className="container-x max-w-3xl py-16 md:py-24 space-y-7">
            {article.body.map((b, i) => {
              ...
              if (b.type === "p") return <p key={i} className="text-base md:text-lg leading-[1.85] text-foreground/80">{b.text}</p>;
              if (b.type === "ul") return (
                <ul key={i} className="space-y-3 border-l-2 border-primary/40 pl-6">
                  {b.items?.map((li, j) => <li key={j} className="text-base text-foreground/80 leading-relaxed">{li}</li>)}
                </ul>
              );
          ...
          <div className="container-x max-w-3xl border-t border-border/60 pt-10 pb-4">
            <Link to={`/insights/${next.slug}`} className="group flex items-center justify-between gap-6 rounded-3xl border border-border/60 p-6 shadow-soft transition hover:border-primary/60">
              <div>
