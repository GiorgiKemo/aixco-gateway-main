      ...
      <div className="container-x">
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          {dubaiFunds.map((f, idx) => (
            <article
              key={f.id}
              onPointerEnter={() => setHoveredFund(f.id)}
              onPointerMove={() => setHoveredFund(f.id)}
              onPointerLeave={() => setHoveredFund((current) => current === f.id ? null : current)}
              onMouseEnter={() => setHoveredFund(f.id)}
              onMouseLeave={() => setHoveredFund((current) => current === f.id ? null : current)}
              className="group relative overflow-hidden rounded-3xl border border-border/60 bg-background shadow-soft transition-all hover:border-primary/40"
            >
              <div className="relative aspect-[16/10] overflow-hidden">
                <img
                  ...
                  width={1536}
                  height={960}
                  className={`media-hover h-full w-full object-cover ${hoveredFund === f.id ? "is-hovered" : ""}`}
                />
                <span className={`absolute top-4 left-4 rounded-full px-2.5 py-1 text-[10px] uppercase tracking-widest ${f.status === "Live" ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground"}`}>
                  {f.status}
                </span>
